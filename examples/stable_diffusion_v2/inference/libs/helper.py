...
class VaeImageProcessor:
    ...
    def resize(self, image, height, width, resample=None):
        ...
        resample = resample if self.resample is not None else self.resample
        image = image.resize((width, height), resample=PIL_INTERPOLATION[self.resample])
        return image
    ...
    def preprocess(self, image, height, width):
        ...
        if isinstance(image[0], PIL.Image.Image):
            ...
            if self.do_resize and (
                    height % self.vae_scale_factor != 0 or width % self.vae_scale_factor != 0
            ):
                raise ValueError(
