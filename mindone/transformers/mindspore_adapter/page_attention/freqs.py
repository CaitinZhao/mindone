...
class FreqsMgr(nn.Cell):
    ...
    def __init__(
        ...
        self.freqs_cos = ms.Tensor(freqs_cos, dtype=rotary_dtype)
        self.freqs_sin = ms.Tensor(freqs_sin, dtype=rotary_dtype)
        self.swap_mask = ms.Tensor(swap_mask, dtype=rotary_dtype)
    ...
    def construct(self, seq_length=None, seq_chunk=None):
        ...
        if self.seq_pipe:
            seg_seq_range = self.seq_seg_range + self.seq_seg_len * seq_chunk
            freqs_cos = mint.gather(self.freqs_cos, seg_seq_range, 0)
            freqs_sin = mint.gather(self.freqs_sin, seg_seq_range, 0)
        else:
            freqs_cos = self.freqs_cos[:seq_length, : self.head_dim]
            freqs_sin = self.freqs_sin[:seq_length, : self.head_dim]
    ...
    def prefill(self, bs, seq_length):
        ...
        freqs_cos = self.freqs_cos[:seq_length, : self.head_dim].tile((bs, 1))
        freqs_sin = self.freqs_sin[:seq_length, : self.head_dim].tile((bs, 1))
        return freqs_cos, freqs_sin, self.swap_mask

    def increment(self, batch_valid_length):
        indices = batch_valid_length - 1
        freqs_cos = mint.gather(self.freqs_cos, indices, 0)
        freqs_sin = mint.gather(self.freqs_sin, indices, 0)
        return freqs_cos, freqs_sin, self.swap_mask
    ...
    def chunk_with_decode(self, seq_range):
        ...
        freqs_cos = mint.gather(self.freqs_cos, seq_range, 0)
        freqs_sin = mint.gather(self.freqs_sin, seq_range, 0)
        return freqs_cos, freqs_sin, self.swap_mask
