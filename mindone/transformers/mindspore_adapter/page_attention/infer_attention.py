...
class InferRotaryEmbedding(nn.Cell):
    ...
    def construct(self, query: ms.Tensor, key: ms.Tensor, freqs_cis, batch_valid_length):
...
class InferAttention(nn.Cell):
    ...
    Inputs:
        - **query** (ms.Tensor[float16, bfloat16]) - The query tensor.
          Input tensor of shape :math:`(B, S1, H1)` or :math:`(B, N1, S1, D)`.
        - **key** (ms.Tensor[float16, bfloat16]) - The key tensor.
          Input tensor of shape :math:`(B, S2, H2)` or :math:`(B, N2, S2, D)`.
        - **value** (ms.Tensor[float16, bfloat16]) - The value tensor.
          Input tensor of shape :math:`(B, S2, H2)` or :math:`(B, N2, S2, D)`.
        - **batch_valid_length** (ms.Tensor) - Int32 tensor with shape [batch_size] the past calculated the index.
          Used for incremental prediction when the use_past is True. Default None.
        - **block_tables** (ms.Tensor[int64]) - Store mapping tables for each sequence.
        - **slot_mapping** (ms.Tensor[int32]) - Store token cache physical slot index.
        - **freqs_cos** (ms.Tensor[float16, bfloat16]) - The precompute freqs cos for rotary position embedding used in
          attention, shape is (seq_len, head_dim).
        - **freqs_sin** (ms.Tensor[float16, bfloat16]) - The precompute freqs sin for rotary position embedding used in
          attention, shape is (seq_len, head_dim).
        - **attn_mask** (Union[ms.Tensor[uint8], None]) - The attention mask tensor. For each element, 0 indicates
          retention and 1 indicates discard. Input tensor of shape :math:`(B, N1, S1, S2)`, :math:`(B, 1, S1, S2)`,
          :math:`(S1, S2)` or (2048, 2048).
        - **alibi_mask** (Union[ms.Tensor[float16, bfloat16], None]) - The position embedding code. If S is greater than
        ...
        - **prefix_keys_values** (Union[ms.Tensor[float16, bfloat16], None]) - The prefix keys values.
        - **q_seq_lens** (Union[ms.Tensor[int32], None]) - The query actual seq len.

    Outputs:
        - **attention_out** (ms.Tensor[float16, bfloat16]) - The output of attention, its shape, and data type
    ...
    def __init__(
        ...
        self.inv_norm_factor = ms.Tensor(1.0 / math.sqrt(self.head_dim), dtype=compute_dtype)
    ...
    def _core_attention(self, query, key, value, attn_mask, alibi_mask=None):
        ...
        Outputs:
            weighted_values: ms.Tensor, the weighted sum scores
        """
        ...
        if alibi_mask is not None:
            score = score + alibi_mask
        score = attn_mask + score

        attention_probs = score.to(ms.float32).softmax()
        # score, v: [bs, n_head, seq/1, seq], [bs, n_head, seq, head_dim]
        weighted_values = self.batch_matmul(attention_probs.to(self.compute_dtype), value)
    ...
    def _repeat_kv(self, x, rep):
        ...
        x = x.reshape((bs, n_kv_head, 1, seqlen * head_dim))
        x = x.tile((1, 1, rep, 1))
        x = x.reshape((bs, n_kv_head * rep, seqlen, head_dim))
        return x
    ...
    def _merge_heads(self, x):
        ...
        """
        # [bs, n_head, seq/1, head_dim]
        x = x.transpose((0, 2, 1, 3))  # dp,mp,1,1 -> dp,1,mp,1
    ...
    def _cat_prefix(self, key, value, prefix_keys_values):
        ...
        if prefix_keys_values is not None:
            ...
            past_key = past_key.to(key.dtype)
            past_value = past_value.to(value.dtype)
            key = mint.cat((past_key, key), 1)
            value = mint.cat((past_value, value), 1)
        return key, value
    ...
    def _core_attention_bsh(self, query, key, value, attn_mask, alibi_mask):
        ...
        query = query.reshape((bs, -1, self.n_head, self.head_dim)).transpose((0, 2, 1, 3))
        key = key.reshape((bs, key_seq_len, self.n_kv_head, self.head_dim)).transpose((0, 2, 1, 3))
        value = value.reshape((bs, value_seq_len, self.n_kv_head, self.head_dim)).transpose((0, 2, 1, 3))
